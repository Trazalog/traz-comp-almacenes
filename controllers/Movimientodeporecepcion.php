  ...
  /**
	...
	*/
	public function traerRecepciones()
	{
		...
		$otralist = $this->Movimientodeporecepcion->Get_Recpciones($id_dest_esta, $id_dest_depo);
	...
	public function traerLotes(){
		...
		$resp = $this->Movimientodeporecepcion->traerLotes($arti_id, $depo_id);
	...
	*/
	public function guardar(){
		...
		$resp = $this->Movimientodeporecepcion->guardarCabecera($cabecera);
		if (!$resp) {
	...
	*/
	function updateLotesDetalles($detalote){
		...
		foreach ($detalote as $value) {
			...
			$resp_set_lote = $this->Movimientodeporecepcion->actualizarLotes($lotes);
			if(!$resp_set_lote){
			...
			//FIXME: ACA NO TRAE EL LOTE GENERADO Y SE CORTA
			//traer id de lote
			...
			$resp_detalle = $this->Movimientodeporecepcion->guardarDetalle($detalle);
			if (!$resp_detalle) {
	...
	*/
	function getLoteIdGenerado($data){
		...
		$lote_id = $this->Movimientodeporecepcion->getLoteIdGenerado($prov_id, $arti_id, $depo_id, $cod_lote);
		if ($lote_id == null) {
